...
for(rd in c(rawData)) #,rawData2)) # Loop through combines, xx and xy datasets
{
	...
	for(experiment in genotypes) # Experiment name - what method we used to create SIS identifier gene list
  {
	  ...
	  if(file.exists(targetcacheFile))
	  {
	    ...
	    message(paste0("\t\t\t\tLoading fitted model - ",add(targetcacheFile,'_model')))
	    
	    m<-readRDS(file=add(targetcacheFile,'_model'))
        ...
        #g<-c('Sertoli','Sertoli_Cycling','Pre-Sertoli','Pre-Supporting_1','Granulosa',"Pre-Supporting_3")
	      ...
	      #pData(cds)$confused_old<-(cds$sism>=cutoffgx)&((cds$sism<=cutoffsx)|(cds$sisf>=cutoffsy))

	      a<-10    # Hmmm....
              b<-cutoffgx*2     # 2x end of nub position

        pData(cds)$confused<-((cds$sisf+cds$sism)^2/(b^2)-(cds$sisf-cds$sism)^2/(a^2)>=1)&(cds$sismf>0)
	    ...
	    {
	      newFile<-add(rd,paste0("_",algorithm,"_",mode))
