...
for(cutmode in c('global_cut')) #,'sg_cut'))
{
  for(cut in c(.95)) # ,.975,.99)) # Graph cutoff v numbers v P
  {
    ...
    for(pass in 1:3)
    {
      ...
      celltype<-'all'
        ...
        ldp<-sum(z2[z2$genotype=='xy_hom','confused'])
        lt<-sum(!z2[z2$genotype=='xy_hom','confused'])
        
        adp<-sum(z2[z2$genotype=='xy','confused'])
        at<-sum(!z2[z2$genotype=='xy','confused'])
        ...
        if(pass==2)
        {
          message(paste(cut,celltype,p,sep='\t'))
          ...
          if(celltype %in% c("Sertoli","Granulosa","Pre-Sertoli"))
          {
            for(genotype in c('xy','xx','xy_hom'))
            {
              ...
              if(nrow(z3)>1)
              {
                ...
                suppressMessages(ggsave(file=paste0(graph,"/ct_","_",celltype,"_",genotype,"_",cutmode,"_",cut,".png"),p))
      ...
      for(genotype in c('xy','xx','xy_hom'))
      {
        ...
        if(nrow(z3)>1)
        {
          ...
          ggsave(file=paste0(graph,"/ct_","_interesting_",genotype,"_",cut,".png"),p,width = 9,height=7)
    ...
    ggsave(file=paste0(graph,"/ct_","_interesting_",cutmode,"_",cut,".png"),grid.arrange(g1,g2,g3,g4,g5,g6,nrow=2,ncol=3),width = 24,height=12)
  ...
  ggsave(file=paste0(graph,"/ct_","_interesting_",cutmode,".png"),do.call("grid.arrange",c(pl,ncol=length(pl)/3)),width = 1920/1080*12,height=12)
}
